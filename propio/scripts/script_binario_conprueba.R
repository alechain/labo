...
ArbolSimple  <- function( fold_test, data, param )
{
  ...
  ganancia_testing  <- sum(  data[ fold==fold_test ][ prob_baja2 >0.05,  ifelse( clase_ternaria=="BAJA+2", kTPGain, kFPGain ) ] )
...
base[,.(media_b=mean(SI),min_b=min(SI),maximo_b=max(SI), media_a=mean(NO),min_a=min(NO),maximo_a=max(NO)), by=tr]

prob_baja2  <- prediccion[, "SI"]

ganancia_testing  <- sum(  dtrain[ prob_baja2 >0.05,  ifelse( clase_ternaria=="BAJA+2", kTPGain, kFPGain ) ] )
