...
tb_hojas  <-  dataset[  , list( reg_rest = (sum( ifelse( !(foto_mes  %in% (202103)), 1, 0 ) ))/28,
                                reg_202103 = sum( ifelse( foto_mes==202103, 1, 0 ) ),
                                b2_rest  = (sum( ifelse( !(foto_mes  %in% (202103)) & clase_ternaria=="BAJA+2", 1, 0 ) ))/28,
                                b2_202103  = sum( ifelse( foto_mes==202103 & clase_ternaria=="BAJA+2", 1, 0 ) ),
                                gan_rest = (sum( ifelse( !(foto_mes  %in% (202103)), gan, 0 ) ))/28,
...
tb_hojas[  gan_rest>0,
           list( sum( gan_rest ),  sum( gan_202103 ) ) ]
